#!/usr/bin/env bash
# Verifies everything in claude-code-avatar/README.md that does NOT require the `claude` binary.
...
[[ -f claude-code-avatar/CLAUDE.md ]] || fail "missing CLAUDE.md"
...
if command -v claude >/dev/null 2>&1; then
  echo "== claude found: $(claude --version 2>/dev/null || true)"
else
  echo "== claude not in PATH (expected in CI; install per README §1–3 on your Mac)"
fi
